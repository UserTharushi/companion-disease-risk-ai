...
- `clinic-service` (port 4003) — clinics/surgeons/slots/appointments with booking conflict checks and `GET /api/clinics/nearby?lat=&lng=` (haversine, merged with live OpenStreetMap POIs). Clinic/surgeon/slot writes require vet/admin role. Also owns `PetAccessGrant` and `/api/access-grants`. Demo clinic seeding is **opt-in** via `SEED_DEMO_DATA=true` (off by default — the seeded rows are invented). A `Surgeon` may carry `userId` linking the clinic listing to a veterinarian's auth account.
  - **Inquiries are bounded threads**, not one-shot questions and not a chat. `messages[]` is the source of truth (`message` is a retained mirror of the first turn); `status` is *derived* from the last speaker — `awaiting_vet` / `answered` / `closed` — so an owner follow-up automatically re-surfaces the thread in the vet's queue. `MAX_THREAD_MESSAGES` (10) closes the thread and points the owner at a real appointment; this cap is a deliberate triage boundary, so don't lift it into an open-ended chat. Append via `POST /api/inquiries/:id/messages` (both roles; the sender is taken from the verified identity header, never the body).
  - Inquiry visibility is scoped like everything else: an owner sees their own, a **vet sees only threads addressed to a surgeon listing carrying their `userId`** (fail closed — no linked listing means an empty list), an admin sees all. The same check gates replying.
