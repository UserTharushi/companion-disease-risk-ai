...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center gap-3 overflow-hidden rounded-lg border p-3 shadow-lg transition-all data-[state=open]:animate-slide-up data-[state=closed]:animate-in",
  {
    variants: {
      variant: {
        default: "border-border bg-surface text-accent",
...
>(({ className, ...props }, ref) => (
  <ToastPrimitive.Close
    ref={ref}
    className={cn("absolute right-1.5 top-1.5 rounded p-1 text-accent-faint opacity-0 transition hover:text-accent group-hover:opacity-100", className)}
    {...props}
  >
...
>(({ className, ...props }, ref) => (
  <ToastPrimitive.Description ref={ref} className={cn("text-xs text-accent-subtle", className)} {...props} />
));
