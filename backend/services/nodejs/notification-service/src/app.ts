...
async function connectMongoWithFallback() {
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    for (const uri of candidates) {
      try {
        ...
        console.log(`[notification-service] MongoDB connected (${uri.includes("@mongodb:") ? "docker" : "local/env"})`);
        return;
      } catch (err) {
        lastError = err;
        console.warn(`[notification-service] MongoDB connection failed for URI: ${uri}`);
      }
    }
  ...
  console.error("[notification-service] MongoDB error", lastError);
}
