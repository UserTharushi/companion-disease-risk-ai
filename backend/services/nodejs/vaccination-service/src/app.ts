...
async function connectMongoWithFallback() {
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    for (const uri of candidates) {
      try {
        ...
        console.log(`[vaccination-service] MongoDB connected (${uri.includes("@mongodb:") ? "docker" : "local/env"})`);
        return;
      } catch (err) {
        lastError = err;
        console.warn(`[vaccination-service] MongoDB connection failed for URI: ${uri}`);
      }
    }
  ...
  console.error("[vaccination-service] MongoDB error", lastError);
}
...
app.use((err: Error, _req: express.Request, res: express.Response, _next: express.NextFunction) => {
  console.error("[vaccination-service]", err);
